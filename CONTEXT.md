...
Run tests: `for t in test_foundation test_agent test_evalproto test_subagents test_orchestrator test_cli test_library test_tui test_backends test_seeds; do PYTHONPATH=src .venv/bin/python -m tests.$t; done` (62 checks, all offline except one live GitHub fetch)
...
- Still TODO from the analysis (not yet built): smarter `reflect` cadence (clean-context "research" agent only on plateau/stuck triggers + "judge by ceiling not iteration-0"); persistent on-disk experiment ledger (summary.md + structured result.md) surviving compaction; an `ab_benchmark` paired-rerun tool + one-change-per-iteration discipline.
