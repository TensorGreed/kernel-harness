...
Run tests: `for t in test_foundation test_agent test_evalproto test_subagents test_orchestrator test_cli test_library test_tui test_backends test_seeds test_ledger; do PYTHONPATH=src .venv/bin/python -m tests.$t; done` (66 checks, all offline except one live GitHub fetch)
...
- Still TODO from the analysis (not yet built, in priority order): (1) smarter `reflect` cadence — a clean-context "research" agent fired only on plateau/stuck triggers (5+ exps within 5%, 3 correctness fails, about-to-repeat), reading the ledger via tools, + "judge a direction by its ceiling not iteration-0"; (2) an `ab_benchmark` paired-rerun tool + one-change-per-iteration discipline — lowest priority on the single GB10 (low run-to-run noise), revisit only if the first real run shows it's needed.
